...
TEST_CASE("empty input results in 0", "[basic]") {
  REQUIRE(hw_II::calc_max_score({}) == 0);
}

TEST_CASE("single input results in it's value", "[basic]") {
  REQUIRE(hw_II::calc_max_score({0}) == 0);
  REQUIRE(hw_II::calc_max_score({1}) == 1);
  REQUIRE(hw_II::calc_max_score({-1}) == -1);
}

TEST_CASE("some trivial input ", "[basic]") {
  REQUIRE(hw_II::calc_max_score({2, 1}) == 2);
  REQUIRE(hw_II::calc_max_score({1, 2}) == 2);
  REQUIRE(hw_II::calc_max_score({1, 2, 3}) == 5);
}

TEST_CASE("example", "[basic]") {
  REQUIRE(hw_II::calc_max_score({1, 2, 4, 2, 6}) == 12);
}

TEST_CASE("non trivial input", "[advanced]") {
  REQUIRE(hw_II::calc_max_score({8, 7, 4, 2, 5}) == 20);
}

TEST_CASE("bigger input", "[stress]") {
  REQUIRE(hw_II::calc_max_score({331,  -813, 380, 208,  352,  110,
                                 -801, -675, 916, -480, -448, 616,
                                 785,  275,  894, 987,  -160, 959,
                                 459,  900,  597, -701, -154, 310}) == 8134);
}

TEST_CASE("even bigger input", "[stress]") {
  REQUIRE(hw_II::calc_max_score(
              {912,  662,  244,  -356, 96,   184,  -491, -535, -407, 185,  352,
...
TEST_CASE("ultimate stress", "[ultimate_stress]") {
  REQUIRE(hw_II::calc_max_score(
              {-189,  -308,  28,    -505,  -31,  -814, -50,   -858, -235,  611,
